...
const TEAMS: TeamDef[] = [
  {
    ...
    managerRole: 'EDITOR_MANAGER', memberRole: 'EDITOR_MEMBER',
    managerLabel: 'Team Manager', memberLabel: 'Team Member',
  ...
  },
  {
    ...
    managerRole: 'VERIFICATION_MANAGER', memberRole: 'VERIFICATION_MEMBER',
    managerLabel: 'Verification Manager', memberLabel: 'Fact Checker',
  ...
  },
  {
    ...
    managerRole: 'REPORTER_APP_MANAGER', memberRole: 'REPORTER_APP_MEMBER',
    managerLabel: 'App Manager', memberLabel: 'App Moderator',
  ...
  },
  {
    ...
    managerRole: 'REPORTERS_MANAGER', memberRole: 'REPORTERS_MEMBER',
    managerLabel: 'Reporters Manager', memberLabel: 'Support Member',
    permissions: ['Verify reporter identities', 'Issue Press ID cards', 'Manage reward payouts', 'Handle reporter accounts'],
  },
  {
    id: 'ADVERTISEMENT_TEAM',
    label: 'Advertisement Manager',
    labelTa: 'விளம்பர மேலாளர் குழு',
    description: 'Manage overall advertising strategy and campaigns',
    icon: <Megaphone size={18} />,
    color: 'text-pink-600', bgColor: 'bg-pink-50', borderColor: 'border-pink-200',
    managerRole: 'ADVERTISEMENT_MANAGER', memberRole: 'LOCAL_ADS_MANAGER',
    managerLabel: 'Ad Manager', memberLabel: 'Local Ads Manager',
    permissions: ['Manage all ad campaigns', 'Set ad priorities', 'View analytics', 'Configure placement'],
  },
  {
    id: 'LOCAL_ADS_TEAM',
    label: 'Local Ads Team',
    labelTa: 'உள்ளூர் விளம்பர குழு',
    description: 'Create and manage local business advertisements',
    icon: <Tv2 size={18} />,
    color: 'text-teal-600', bgColor: 'bg-teal-50', borderColor: 'border-teal-200',
    managerRole: 'LOCAL_ADS_MANAGER', memberRole: 'LOCAL_ADS_MANAGER',
    managerLabel: 'Local Ads Manager', memberLabel: 'Local Ads Member',
    permissions: ['Create local ads', 'Upload ad media', 'Set CTA links', 'Monitor ad performance'],
  },
  {
    id: 'ADMOB_TEAM',
    label: 'AdMob Manager',
    labelTa: 'AdMob மேலாளர் குழு',
    description: 'Configure and manage Google AdMob settings',
    icon: <RadioTower size={18} />,
    color: 'text-amber-600', bgColor: 'bg-amber-50', borderColor: 'border-amber-200',
    managerRole: 'ADMOB_MANAGER', memberRole: 'ADMOB_MANAGER',
    managerLabel: 'AdMob Manager', memberLabel: 'AdMob Member',
    permissions: ['Configure AdMob IDs', 'Set ad placements', 'View revenue reports', 'Manage test devices'],
  },
];
...
const ROLE_META: Record<string, { label: string; badge: string }> = {
  ...
  ADVERTISEMENT_MANAGER:{ label: 'Ad Manager',            badge: 'bg-pink-50 text-pink-700 border border-pink-200' },
  LOCAL_ADS_MANAGER:    { label: 'Local Ads Manager',     badge: 'bg-teal-50 text-teal-700 border border-teal-200' },
  ADMOB_MANAGER:        { label: 'AdMob Manager',         badge: 'bg-amber-50 text-amber-700 border border-amber-200' },
};

function RoleBadge({ role }: { role: string }) {
  const m = ROLE_META[role] ?? { label: role, badge: 'badge-gray' };
          ...
          {menuOpen && (
            <>
              ...
              <div className="absolute right-0 top-8 z-20 bg-surface border border-border rounded-lg shadow-lg py-1 w-44">
                ...
                <button onClick={() => { setMenuOpen(false); onToggleActive(member.id, !(member.isActive !== false)); }} className="flex items-center gap-2 w-full px-3 py-2 text-xs text-text-primary hover:bg-page">
...
function TeamCard({ team, members, currentAdminId, onAddMember, onEdit, onDelete, onToggleActive }: {
  ...
  const teamMembers = members.filter(m => m.adminRole === team.memberRole && m.adminRole !== team.managerRole);
    ...
    <div className={`card overflow-hidden border-l-4 ${team.borderColor}`}>
      <div className="card-header cursor-pointer select-none" onClick={() => setExpanded(!expanded)}>
        ...
        <div className="flex items-center gap-2">
          <button onClick={(e) => { e.stopPropagation(); onAddMember(team); }}
        ...
        <div>
          <div className="px-4 py-2 bg-page border-b border-border flex flex-wrap gap-2">
            {team.permissions.map(p => (
              <span key={p} className="text-2xs text-text-muted bg-surface border border-border px-2 py-0.5 rounded">✓ {p}</span>
            ))}
          </div>
...
export default function AccountsPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setCreateTeam(null); setShowSysCreate(false); createForm.reset();
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Account deleted'); qc.invalidateQueries({ queryKey: ['admins'] }); setDeleteId(null); },
    onError: () => toast.error('Failed to delete'),
  });

  const toggleActiveMutation = useMutation({
    mutationFn: ({ id, isActive }: { id: string; isActive: boolean }) => apiPatch(`/admin/accounts/${id}`, { isActive }),
  ...
  function openTeamAdd(team: TeamDef, role: 'manager' | 'member' = 'member') {
    ...
    createForm.reset({ adminRole, team: team.id as TeamType });
  }
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2">
              {(['SUPER_ADMIN', 'ADMIN'] as AdminRole[]).map(r => (
                <button key={r} type="button" onClick={() => createForm.setValue('adminRole', r)}
                  className={`flex-1 py-2 px-3 rounded-lg border text-xs font-medium transition-all ${createForm.watch('adminRole') === r ? 'bg-red/10 border-red/30 text-red' : 'border-border text-text-secondary hover:bg-page'}`}>
        ...
        <Modal title="Edit Account" onClose={() => setEditing(null)}>
          <form onSubmit={editForm.handleSubmit((v) => editMutation.mutate({ id: editing.id, payload: v }))} className="space-y-4">
            ...
            <div>
              ...
              <input {...editForm.register('phone')} type="tel" className="input-field" placeholder="+91 98765 43210" />
            </div>
            <div>
            ...
            <div className="flex items-center justify-between py-2 border-t border-border">
              ...
              <button type="button" onClick={() => editForm.setValue('isActive', !editForm.watch('isActive'))}
...
function CreateAccountFields({ form, showPass, setShowPass, onSubmit, isPending, onCancel }: {
  form: ReturnType<typeof useForm<CreateForm>>;
  showPass: boolean; setShowPass: (v: boolean) => void;
  onSubmit: (v: CreateForm) => void; isPending: boolean; onCancel: () => void;
}) {
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button type="submit" disabled={isPending} className="btn-primary flex-1">
          {isPending && <Loader2 size={14} className="animate-spin" />} Create Account
        </button>
      </div>
    </form>
  );
